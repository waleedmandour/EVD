...
export function BottomNav() {
  const { connectionMode, vehicleData } = useAppStore();
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-[#0a0f1a]/95 backdrop-blur-lg border-t border-slate-800/50">
      <div className="max-w-md mx-auto flex items-center justify-around py-2 px-2">
        <NavItem tab="dashboard" icon={<svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>} label="Dashboard" />
        ...
        <NavItem tab="trip" icon={<svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 8V4H8"/><rect x="4" y="4" width="16" height="5" rx="1"/><path d="M12 13V9"/><rect x="6" y="9" width="12" height="4" rx="1"/><circle cx="12" cy="20" r="2"/><path d="M8 20h8"/></svg>} label="Trip" />
        <NavItem tab="controls" icon={<svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 2v4m0 12v4M4.93 4.93l2.83 2.83m8.48 8.48l2.83 2.83M2 12h4m12 0h4M4.93 19.07l2.83-2.83m8.48-8.48l2.83-2.83"/></svg>} label="Controls" />
      </div>
    </nav>
  );
}
...
function NavItem({ tab, icon, label }: { tab: string; icon: React.ReactNode; label: string }) {
  // We use URL hash to manage active tab (client-side only)
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-slate-500">
              {connectionMode === 'demo' ? 'DEMO' : connectionStatus === 'connected' ? 'BT' : 'OFF'}
            </span>
          </div>
